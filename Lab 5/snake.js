...
(function(){
	...
	class SnakePart {
		constructor(part) {
			this.part = part;
			// we want to use the computed style, not the style property of the element to get the most up to date values
	...
	class Snake {
		...
		isFoodEaten() {
			// parseInt() takes a string and returns the first complete integer it can find in that string
			const foodTop = parseInt(food.compStyle.top);
			const snakeTop = parseInt(this.snakeParts[0].compStyle.top);
			const foodLeft = parseInt(food.compStyle.left);
			const snakeLeft = parseInt(this.snakeParts[0].compStyle.left);

			return foodTop === snakeTop && foodLeft === snakeLeft;
